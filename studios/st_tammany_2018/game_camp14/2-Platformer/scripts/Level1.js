class Level1 extends Phaser.Scene
{
    ...
    create_map()
    {
     ...
     setup_physics()
     {
        ...
        this.physics.add.overlap(this.player, this.group_collectables, this.d_take, null, this);
        this.physics.add.overlap(this.player, this.group_goal, this.next_scene, null, this);

    }
    setup_camera()
    {
       this.cameras.main.startFollow(this.player); 
       this.cameras.main.setBackgroundColor('rgb(210,210,210)')
    }
    ...
    create_objects()
    {
        ...
        this.group_hazard1 = this.map.createFromObjects('items', 'c', hazard1_image);
        this.setup_objects(this.group_hazard1)
        
        
        var hazard2_image = new Object();
        hazard2_image.key = 'itemMap';
        hazard2_image.frame = 3;
        this.group_hazard2 = this.map.createFromObjects('items', 'a', hazard2_image);
        this.setup_objects(this.group_hazard2)
        
        var collect_image = new Object();
        collect_image.key = 'itemMap';
        collect_image.frame = 2;
        this.group_collectables = this.map.createFromObjects('items', 'd', collect_image);
        this.setup_objects(this.group_collectables)
        
        var goal_image = new Object();
        goal_image.key = 'itemMap';
        goal_image.frame = 4;
        this.group_goal = this.map.createFromObjects('items', 'f', goal_image)
        this.setup_objects(this.group_goal)
        
        var hazard3_image = new Object
        hazard3_image.key = 'itemMap';
        hazard3_image.frame = 0;
        this.group_hazard3 = this.map.createFromObjects('items', 't', hazard3_image);
